...
class ViewController: UIViewController{
    // 프리뷰 용의 뷰와 셔터 버튼을 Outlet 접속해 둔다
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if (authStatus == .authorized)&&(inOutStatus == .ready){
            ...
            // 카메라 이용이 허가되지 않을 때（처음 실행 때는 무효）
            shutterButton.isEnabled = false
            // 알람을 표시한다
            showAlert(appName: "카메라 ")
        }
        // 디바이스가 회전했을 때 통지되는 이벤트 핸들러를 설정한다
    ...
    @IBAction func takePhoto(_ sender: Any) {
        if (authStatus == .authorized)&&(inOutStatus == .ready){
            ...
            // 카메라 이용을 허가하지 않았어도 변함없이 카메라를 탭했다（첫 실행 시만）
            showAlert(appName: "카메라 ")
        }
    }
    ...
    func showAlert(appName:String){
        ...
        let aMessage = "설정＞" + appName + " 이용을 허가해주세요."
